  ...
  */
  ...
  When a response has been received, this input will reset.">
    ...
    (or hit &lt;enter&gt; from the input)">Send</button>
      <button onclick="sendTriangle()" class="send-button" type="button" title="Quick: Set Triangle wave (50% pulse)">Tri</button>
      <button onclick="sendSaw()" class="send-button" type="button" title="Quick: Set Sawtooth wave (1% pulse)">Saw</button>
  ...
  // Quick send: set triangle (triangle generator with 50% pulse width)
  function sendTriangle() {
    var AJAX = new XMLHttpRequest();
    AJAX.onreadystatechange = function() {
      if(this.readyState == 4 && this.status == 200) {
        if (this.responseText) postMSG(this.responseText);
        window.setTimeout( function() { getCommandStatus(); }, updateDelay);
      }
    };
  ...
  // Quick send: set sawtooth (triangle generator with a short rise)
  function sendSaw() {
    var AJAX = new XMLHttpRequest();
    AJAX.onreadystatechange = function() {
      if(this.readyState == 4 && this.status == 200) {
